...
class LinqIterator implements \IteratorAggregate
{
    ...
    public static function asIteratorAggregate(&$traversable)
    {
        ...
        throw new \Exception('asIteratorAggregate cannot convert type to interatoraggregate');
    }
    ...
    public function SelectMany($collectionSelector, $resultSelector = NULL)
    {
        ...
        $newiterfunc = function() use ($currentiterfunc, $collectionSelector, $resultSelector)
        {
            return new \RecursiveIteratorIterator(new SelectManyIterator($currentiterfunc(), $collectionSelector, $resultSelector));
        };
    ...
    {
        $currentiterfunc = $this->getIteratorFunction;
        $newiterfunc = function() use ($currentiterfunc, $wherefunc)
        {
            return new WhereIterator($currentiterfunc(), $wherefunc);
        };
        return new LinqIterator($newiterfunc);
    }
    ...
    {
        $currentiterfunc = $this->getIteratorFunction;
        $newiterfunc = function() use ($currentiterfunc, $selector)
        {
            return new TransformIterator($currentiterfunc(), $selector);
        };
        return new LinqIterator($newiterfunc);
    }
    ...
    public function Take($count)
    {
        ...
        $newiterfunc =
                ...
                {
                    return new TakeWhileIterator($currentiterfunc(), function($val, $idx) use ($count)
                        {
                            return $idx < $count;
                        });
                };
    ...
    public function Skip($count)
    {
        ...
        $newiterfunc =
                function() use ($currentiterfunc, $count)
                {
                    return new SkipWhileIterator($currentiterfunc(), function($val, $idx) use ($count)
                        {
                            return $idx < $count;
                        });
                };
